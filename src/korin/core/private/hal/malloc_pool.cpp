...
MemoryPool::MemoryPool(const SetupData & inSetupData, void * inBuffer)
	: buffer{inBuffer}
	, hasOwnBuffer{inBuffer == nullptr}
	, head{nullptr}
	, numFreeBlocks{inSetupData.numBlocks}
	, numBlocks{inSetupData.numBlocks}
	, blockDataSize{inSetupData.dataSize}
	, blockAlignment{inSetupData.dataAlignment}
	, blockSize{PlatformMath::align2Up((inSetupData.dataSize + blockLogicSize) - 1, inSetupData.dataAlignment)}
{
	if (hasOwnBuffer)
	{
...
{
	CHECKF(isBlockInRange(block), "Block %p was not acquired by this pool", block)
